...
TRAINJSON = '/opt/ml/input/data/paper/train_paper_1.json'
VALJSON = '/opt/ml/input/data/paper/val_paper_1.json'
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Dataset -> mmsegmentation format')
...
def copy_images(json_path, target_path):
    ...
    print(f'image copy to {target_path}!')
...
def create_mask(json_path, target_path):
    ...
    for _, masks, image_infos in data_loader:
        mask = masks[0].numpy()
        img_png = Image.fromarray(mask).convert('P')
        img_png.putpalette(category_list)
        image_info = image_infos[0]        
        img_png.save(os.path.join(target_path, f"{image_info['id']:04}.png"))
